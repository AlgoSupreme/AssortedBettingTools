...
# 1. THE DATA
# ---------------------------------------------------------
if os.path.exists(os.path.join("data_dump", f"goalie_analysis_sog_sv_gl{datetime.now().date()}.json")):
    file_path = os.path.join("data_dump", f"goalie_analysis_sog_sv_gl{datetime.now().date()}.json")
else:
    print("Missing today's data, please run `data_dump.bat` or `py data_dump_goalie.py`")
    input()
    quit()
